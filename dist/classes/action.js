...
class KaminoAction {
    ...
    }, referrer = (0, kit_1.none)(), currentSlot = 0n, payer = owner, overrideElevationGroupRequest = undefined // if set, when an elevationgroup request is made, it will use this value
    ) {
        const axn = await KaminoAction.initialize('deposit', amount, mint, owner, kaminoMarket, obligation, referrer, currentSlot, payer);
    ...
    initUserMetadata = {
        skipInitialization: false,
        skipLutCreation: false,
    }, referrer = (0, kit_1.none)(), currentSlot = 0n, payer = owner, overrideElevationGroupRequest, 
    // Optional customizations which may be needed if the obligation was mutated by some previous ix.
    obligationCustomizations) {
        const axn = await KaminoAction.initialize('withdraw', amount, mint, owner, kaminoMarket, obligation, referrer, currentSlot, payer);
        const addInitObligationForFarm = true;
        if (extraComputeBudget > 0) {
            axn.addComputeBudgetIx(extraComputeBudget);
        }
        ...
        if (!['mint', 'redeem'].includes(action)) {
            ...
            if (!ownerUserMetadata && !initUserMetadata.skipInitialization) {
                ...
                if (!initUserMetadata.skipLutCreation) {
                    const [createLutIx, lookupTableAddress] = await (0, utils_1.createLookupTableIx)(this.kaminoMarket.getRpc(), this.payer);
    ...
    async addAtaIxs(action) {
        ...
        if ((action === 'withdraw' || action === 'borrow' || action === 'redeem') && this.mint !== utils_1.WRAPPED_SOL_MINT) {
            ...
            const [, createUserTokenAccountIx] = await (0, utils_1.createAssociatedTokenAccountIdempotentInstruction)(this.payer, this.reserve.getLiquidityMint(), this.owner.address, this.reserve.getLiquidityTokenProgram(), reserveAta);
        ...
        if (action === 'liquidate') {
            ...
            const [, createUserTokenAccountIx] = await (0, utils_1.createAssociatedTokenAccountIdempotentInstruction)(this.payer, this.outflowReserve.getLiquidityMint(), this.owner.address, this.outflowReserve.getLiquidityTokenProgram(), outflowReserveAta);
            this.setupIxs.unshift(createUserTokenAccountIx);
            ...
            const [, createUserCollateralAccountIx] = await (0, utils_1.createAssociatedTokenAccountIdempotentInstruction)(this.payer, this.outflowReserve.getCTokenMint(), this.owner.address, token_1.TOKEN_PROGRAM_ADDRESS, ctokenAta);
        ...
        if (action === 'depositAndBorrow' || (action === 'repayAndWithdraw' && this.secondaryMint !== utils_1.WRAPPED_SOL_MINT)) {
            ...
            if (!additionalUserTokenAccountInfo.exists) {
                const [, createUserTokenAccountIx] = await (0, utils_1.createAssociatedTokenAccountIdempotentInstruction)(this.payer, this.outflowReserve.getLiquidityMint(), this.owner.address, this.outflowReserve.getLiquidityTokenProgram(), additionalUserTokenAccountAddress);
                this.setupIxs.unshift(createUserTokenAccountIx);
        ...
        if (action === 'mint') {
            ...
            const [, createUserCollateralAccountIx] = await (0, utils_1.createAssociatedTokenAccountIdempotentInstruction)(this.payer, this.reserve.getCTokenMint(), this.owner.address, token_1.TOKEN_PROGRAM_ADDRESS, ctokenAta);
            this.setupIxs.unshift(createUserCollateralAccountIx);
