...
exports.DISCRIMINATOR = Buffer.from([107, 73, 15, 119, 195, 116, 91, 210]);
exports.layout = borsh.struct([
    borsh.vec(types.DataSource.layout(), "validDataSources"),
]);
function setDataSources(args, accounts, remainingAccounts = [], programAddress = programId_1.PROGRAM_ID) {
    const keys = [
    ...
    const len = exports.layout.encode({
        validDataSources: args.validDataSources.map((item) => types.DataSource.toEncodable(item)),
    }, buffer);
    const data = Buffer.concat([exports.DISCRIMINATOR, buffer]).slice(0, 8 + len);
    const ix = { accounts: keys, programAddress, data };
    return ix;
}
//# sourceMappingURL=setDataSources.js.map